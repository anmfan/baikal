...
let usedThreads = (Number(process.env.CIRCLE_NODE_TOTAL) || (os.cpus() || { length: 1 }).length) - 1
...
const nextConfig = {
    ...
    webpack: (config) => {
      ...
      config.resolve.alias['@itcase/ui'] = path.join(dirname, 'node_modules/@itcase/ui/dist')
